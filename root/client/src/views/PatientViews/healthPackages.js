  ...
  return (
    <Container className="mt-5">
      <Row>
        ...
        <Col xs="6">
          <Container style={{ marginLeft: "20px", marginTop: "40px", width: "100%" }}>
            <Slider ref={sliderRef} {...settings}>
              {slideContent.map((slide, index) => (
                <div key={index} style={{ marginLeft: "20px", marginBottom: "20px" }}>
                  <div style={{ backgroundColor: "#0C356A", padding: "30px", borderRadius: "8px", color: "#ffffff", width: "300px", height: "300px" }}>
            ...
            <div style={{ textAlign: "left", marginTop: "40px", marginLeft: "15px" }}>
              <Button onClick={handlePrev}>Previous</Button>
              <span style={{ margin: "0 20px" }}>{currentSlide + 1} / {slideContent.length}</span>
              <Button onClick={handleNext}>Next</Button>
              {Array.from({ length: slideContent.length }).map((_, index) => (
                <span key={index} className={index === currentSlide ? "dot active" : "dot"} onClick={() => sliderRef.current.slickGoTo(index)}></span>
              ))}
            </div>
          </Container>
        </Col>
        ...
        <Col xs="6">
          <Col xl="19" style={{ position: "relative", top: "20px", right: "10px" }}>
            <Container fluid style={{ marginBottom: "10px" }}>
              <Card className="bg-secondary shadow shadow w-100">
                <CardHeader className="border-0" style={{ backgroundColor: "#0C356A" }}>
                  <Row className="align-items-center">
                    <Col xs="12">
                      <h6 className="heading-small text-muted mb-4" style={{ fontSize: "24px", fontWeight: "bold" }}>
                ...
                <CardBody style={{ backgroundColor: "#0C356A" }}>
                  <Form>
                    ...
                    <Row>
                      <Col lg="5">
                        <FormGroup>
                          <label className="form-control-label" style={{ color: "#f7fafc" }}>
